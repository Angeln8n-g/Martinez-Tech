...
export const supabaseService = {
  ...
  async getBootstrapData() {
    ...
    const visits: TechnicalVisit[] = (visitsRes.data || []).map(v => ({
      ...
      assignedTechnicianId: v.assigned_technician_id,
      assignedTechnicianEmail: v.assigned_technician_email,
      status: v.status,
      notes: v.notes,
  ...
  async upsertVisit(v: TechnicalVisit) {
    const payload = {
      ...
      date: v.date,
      time: v.time,
      duration_minutes: v.durationMinutes || 60,
      type: v.type,
      assigned_technician: v.assignedTechnician,
      assigned_technician_id: v.assignedTechnicianId,
      assigned_technician_email: v.assignedTechnicianEmail,
      status: v.status,
      notes: v.notes,
